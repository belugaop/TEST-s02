...
from info import ADMINS, AUTH_CHANNEL, AUTH_USERS, CUSTOM_FILE_CAPTION, AUTH_GROUPS, P_TTI_SHOW_OFF, IMDB, \
    SINGLE_BUTTON, SPELL_CHECK_REPLY, IMDB_TEMPLATE, IMDB_DELET_TIME, AUTO_DELETE_TIME, AUTO_DELETE, SUPPORT_CHAT
from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton, CallbackQuery
from pyrogram import Client, filters
from pyrogram.errors import FloodWait, UserIsBlocked, MessageNotModified, PeerIdInvalid
from utils import droplink_url, get_size, is_subscribed, get_poster, search_gagala, temp, get_settings, save_group_settings
...
def droplinkshort(url):
    URL = f'https://droplink.co/api?api={DROPLINK_API}&url={url}&alias=&format=json'
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton('🤖 Updates', url=f"https://telegram.me/{SUPPORT_CHAT}")
        ], [
...
async def advantage_spell_chok(msg):
    ...
    s = await msg.reply("I couldn't find anything related to that\nDid you mean any one of these?",
                    reply_markup=InlineKeyboardMarkup(btn))
    if AUTO_DELETE:
        await asyncio.sleep(AUTO_DELETE_TIME)
        await s.delete()
        ...
        if re.search(pattern, name, flags=re.IGNORECASE):
            ...
            if btn is not None:
                try:
                    if fileid == "None":
                        if btn == "[]":
                            txt = await client.send_message(group_id, reply_text, disable_web_page_preview=True)
                        else:
                            button = eval(btn)
